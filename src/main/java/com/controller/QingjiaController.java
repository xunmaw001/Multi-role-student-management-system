...
public class QingjiaController {
    ...
    public R save(@RequestBody QingjiaEntity qingjia, HttpServletRequest request) {
        try {
            ...
            String role = String.valueOf(request.getSession().getAttribute("role"));
            if ("学生".equals(role) ||"班干部".equals(role)) {
                Integer userId = (Integer) request.getSession().getAttribute("userId");
                qingjia.setYonghuId(userId);
                qingjia.setQingjiaTypes(1);
            }else if(("管理员".equals(role)) || ("任课教师".equals(role)) || ("辅导员".equals(role)) ){
    ...
    */
    public void dictionaryConvert(QingjiaView qingjiaView){
        ...
        if(StringUtil.isNotEmpty(String.valueOf(qingjiaView.getYonghuTypes()))){
            qingjiaView.setYonghuValue(dictionaryMap.get("yonghu_types").get(qingjiaView.getYonghuTypes()));
        }
    }
