...
<script>
    ...
    function setDataRow(item, number) {
        ...
        row.appendChild(qingjiaTypesCell);
            ...
            //理由
        var liyouContentCell = document.createElement('td');
        //如果图文详情包含图片
        if (item.liyouContent && item.liyouContent.indexOf('img') != -1) {
            //暂时只考虑图片+文字 和 文字+图片的情况
            var beginIndex = item.liyouContent.indexOf('<img');
            var endIndex = item.liyouContent.indexOf('>');
                liyouContentCell.innerHTML = item.liyouContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
        } else {
            if (item.liyouContent != null && item.liyouContent != "" && item.liyouContent.length >= 11) {
                    liyouContentCell.innerHTML = item.liyouContent.substring(0, 10) + "...";
            } else {
                    liyouContentCell.innerHTML = item.liyouContent;
            }
        }
        row.appendChild(liyouContentCell);
        ...
        if(window.sessionStorage.getItem('role') == '学生' || window.sessionStorage.getItem('role') == '班干部'){
        ...
        }else if(window.sessionStorage.getItem('role') == '管理员'  || window.sessionStorage.getItem('role') == '辅导员'){
            //修改按钮
            var editBtn = document.createElement('button');
            var editAttr = 'edit(' + item.id + ')';
            editBtn.setAttribute("type", "button");
